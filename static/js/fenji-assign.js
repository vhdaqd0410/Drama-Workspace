...
async function fjSyncMasterWorkload(){
  if(!confirm('将以当前「分集数据」(episode_plan) 为准，统一各项目工作量统计（不会覆盖分集数据），继续？')){
    return;
  }
  toast('📥 正在按分集数据统一工作量...', 'info');
  try{
    const d = await api('POST', '/api/fenji/sync_episode_plan', {});
    if(d && d.ok){
      const synced = d.synced || [];
      const lines = synced.slice(0, 8).map(function(s){ return '  ✓ ' + s.name + '（' + s.episodes + '集，' + s.editor_count + '人）'; }).join('\n');
      toast('✅ 已按分集数据统一 ' + synced.length + ' 个项目工作量（' + (d.total_episodes||0) + ' 集）', 'success');
